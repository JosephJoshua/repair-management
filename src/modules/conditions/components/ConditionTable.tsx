...
import {
  prefetchConditions,
  useConditionsQuery,
} from '../queries/useConditionsQuery';
import ConditionTableRow from '../types/ConditionTableRow';
import DamageTableActions from './ConditionTableActions';
...
const columns: DataTableColumn<ConditionTableRow>[] = [
  {
    accessor: 'damageId',
    hidden: true,
  },
  ...
  },
  {
    ...
    render: (damage) => <DamageTableActions condition={damage} />,
  },
];

const ConditionTable: FC<ConditionTableProps> = ({
  ...
  const { data, error, isError, isFetching, isPreviousData, refetch } =
    useConditionsQuery({
      query: {
        ...
        sortBy: sortStatus.columnAccessor as keyof ConditionTableRow,
  ...
  useEffect(() => {
    ...
    if (!isPreviousData && hasMore) {
      prefetchConditions(queryClient, {
        query: {
